#!/bin/bash

while read line
do
    tlib=$(echo $line | cut -d' ' -f1)
    nlib=$(echo $line | cut -d' ' -f2)
    tbam=$tlib.merged.sorted.RG.bam
    nbam=$nlib.merged.sorted.RG.bam

    qjob=${tlib}_${nlib}.mutect
    uge=$qjob.uge
    [[ -e $uge ]] && rm $uge
    [[ -e $qjob.log ]] && rm $qjob.log
    [[ -e $qjob.err ]] && rm $qjob.err
    [[ -e $qjob.runlog ]] && rm $qjob.runlog
    echo '#!/bin/bash' >> $uge
    echo '#$ -N' $qjob >> $uge
    echo '#$ -o' $qjob.log >> $uge
    echo '#$ -e' $qjob.err >> $uge
    echo '#$ -q medium.q' >> $uge
    echo '#$ -l h_rt=48:00:00,mem_free=30G -pe OpenMP 1' >> $uge
    echo '#$ -cwd' >> $uge
    echo 'source /mnt/software/etc/gis.bashrc' >> $uge
    echo 'source /opt/uge-8.1.7p3/aquila_cell/common/settings.sh' >> $uge
    echo bash muTect.sh $tbam $nbam $tlib $nlib 30G '>>'$qjob.runlog '2>&1' >> $uge
    qsub < $uge
done < tumour_normal_lib
