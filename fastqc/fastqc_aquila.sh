...
do
    ...
    echo '#$ -e' $qjob.err >> $uge
    echo '#$ -q medium.q' >> $uge
    echo '#$ -l h_rt=48:00:00,mem_free=4G -pe OpenMP 4' >> $uge
    echo '#$ -cwd' >> $uge
    echo 'source /mnt/software/etc/gis.bashrc' >> $uge
    echo 'source /opt/uge-8.1.7p3/aquila_cell/common/settings.sh' >> $uge
    echo '[[ -e' ${fastq}_qc ']]' '||' mkdir ${fastq}_fastqc >> $uge # first make the output directory
    echo $fastqc -f fastq --extract -t 4 $fastq '>>'$qjob.runlog '2>&1' >> $uge # no need to extract gz file in advance
    qsub < $uge
done < fastq_list
