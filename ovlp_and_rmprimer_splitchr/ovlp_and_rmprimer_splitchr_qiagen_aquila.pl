...
my $samtools = "/mnt/projects/liuxl/ctso4_projects/liuxl/Tools/samtools/samtools-1.1/bin/samtools";
...
my $fm = new Parallel::ForkManager(1);
while(<CHRS>) {
    chomp;
    my $chr = $_;
    my $child = $fm ->start and next;
    print "$chr send to cluster\n";
    my $qjob = "$input_bam.$chr.ovlprefine_trimprimer_qiagen";
    my $uge = "$qjob.uge";
    `rm $uge` if -e $uge;
